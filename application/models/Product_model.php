	...
	class Product_model extends CI_model{
		...
		**********************************/
		function shop_wise_product($product_id, $exclude)
		{
			...
			//if($exclude) $this -> db -> where('shop_setup.shop_id != "'.$this -> tank_auth -> get_shop_id().'"');
			$this -> db -> where('product_info.product_id = bulk_stock_info.product_id');
			//$this -> db -> where('bulk_stock_info.product_id = sale_price_info.product_id');
			//$this -> db -> where('sale_price_info.shop_id = shop_setup.shop_id');
			//$this -> db -> where('bulk_stock_info.shop_id = sale_price_info.shop_id');
			//$this -> db -> group_by('shop_setup.shop_id');
			return $query = $this -> db -> get();
		}
		...
		function distirbutor($product_id)
		{
			...
			foreach ($query -> result() as $field){
					$data[ $field -> distributor_id ] = $field -> distributor_name .'('.$field -> purchase_receipt_id.')';
				} */
			return $query;
		}
		/****************************
		...
		******************************/
		...
		function catagory_name()
		{
			...
			foreach ($query-> result() as $field){
					//$data[ $field -> catagory_name ] = $field -> catagory_name;
		...
		function product_type()
		{
			...
			$data[''] =  'Select Type';
			foreach ($query-> result() as $field){
					//$data[ $field -> product_type ] = $field -> product_type;
		...
		public function search_purchase_buy_price($product_id)
		{
			...
			$data 	= $this->db
					...
					//->join('purchase_info','(product_info.product_id = purchase_info.product_id AND purchase_info.unit_buy_price != 0)','left')
					->get();
		...
		function create_shelf()
		{
         ...
         function create_store_in_shelf()
		 {
		 	$timezone = "Asia/Dhaka";
			date_default_timezone_set($timezone);
			$bd_date=date('Y-m-d');
			...
			if($shelf_no < 10) $shelf_no = "0".$shelf_no;
			 
			if($row < 10) $row = "0".$row;
			if($column < 10) $column = "0".$column;
			
			//echo $shelf_no.$row.$column;
		    $cell_no = $shelf_no.$row.$column;
	       ************************************/
			$new_store_in_shelf_insert_data = array(
				
				'cell_no' => $shelf_no, //$cell_no,
				'product_id' => $this -> input -> post('product_id'),			
				'product_pos_creator' => $creator,
				'product_pos_doc' => $bd_date
			);
			$insert = $this -> db -> insert('product_position_info', $new_store_in_shelf_insert_data);
			return $insert;
		 }
         ...
         /* to make all shelf_no onclick change */
        function all_shelf_no_onlick()
		{				
		    $this->db->order_by("shelf_no", "asc");
			$query = $this -> db -> get('shelf_info');
			$data[''] =  'Select a Product';
			foreach ($query -> result() as $field)
			{
				$data[base_url().'index.php/product_controller/store_in_shelf_entry/'.$field->shelf_no] = $field -> shelf_no;
			}
			return $data;	
		}
		...
		function product_info()
		{
			...
			$data[''] =  'Select a Product';
			foreach ($query -> result() as $field){
					if( $field -> product_status != 'inactive')
	   ...
	   function get_product_specification( $pro_id )
	   {
		...
		function specific_product_name($pro_id)
		{
			...
			//$product_name = $this->db->where('product_id', $pro_id );
		    // $product_id = array('product_id' => $pro_id);
			//$product_name = $this -> db -> where('product_id',$pro_id); 
			//$query = $this->db->get();
	        //$query = $this -> db -> get('product_info');
		 ...
		 *********************************/
		 function fatch_all_purchase_receipt_id()
		 {
			$shop_id = $this -> tank_auth -> get_shop_id();
		 	$this->db->order_by("receipt_id", "desc");
			//$query = $this -> db -> get('purchase_receipt_info');
		 ...
		 function fatch_all_purchase_receipt_id_for_transaction_purpose()
		 {
			...
			foreach ($query -> result() as $field)
			{
				...
				//$data[base_url().'index.php/account_controller/transaction_entry/'.$segment_3.'/purchase/'.$field->receipt_id] = $field -> receipt_id;
				$data[base_url().'index.php/account_controller/transaction_entry/'.$segment_3.'/purchase/'.$field->receipt_id] = $field -> receipt_id.' (  '.$field -> distributor_name.'  )';	
			}
			return $data;
		 }
