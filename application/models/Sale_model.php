...
class Sale_model extends CI_model
{
	...
	public function getReturnBuyPrice($tmp_sale_id)
	{
		$data = $this->db->select('return_buy_price')->where('temp_sale_id', $tmp_sale_id)->get('temp_sale_info');
		if ($data->num_rows() > 0) {
			$row_data = $data->row();
			return $row_data->return_buy_price;
		} else return 0;
	}
	...
	public function new_active_sale_with_salereturn($return_amount, $return_buy_price)
	{
		...
		$data = array(
			...
			'return_buy_price'      => $return_buy_price,
			'temp_sale_status'      => 1,
		);
		$sql = $this->db
			->select('temp_sale_id')
