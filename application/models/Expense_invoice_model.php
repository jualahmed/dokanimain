...
/******************** 	Previous Query
				...
				{	
					...
					if($expense_type != 'Transport Cost For Purchase')
					{
						...
						foreach ($query -> result() as $field)
						{
							 ...
							 //~ $data[base_url().'index.php/account_controller/transaction_entry/'.$segment_3.'/expense/'.$field->expense_id] = $field -> expense_id.
											//~ ' ( '.'# ' .$field -> expense_type. ' )'.' Name: ( '.': ' .$field -> service_provider_name. ' )';
							$data[base_url().'index.php/account_controller/transaction_entry/'.$segment_3.'/expense/'.$field->expense_id] = $field -> expense_type.' : ( '.$field -> username. ' ) '.' Amount: '.$field -> expense_amount;
						}
					...
					else
					{
						...
						foreach ($query -> result() as $field)
						{
							 ...
							 $data[base_url().'index.php/account_controller/transaction_entry/'.$segment_3.'/expense/'.$field->expense_id] = $field -> expense_type.' Receipt ID: ( '.'# ' .$field -> service_provider_id. ' )'.' ID: '.$field -> expense_id;
						}
					}
				}
				return $data;
        ...
        function fatch_all_invoiceID_not_paid_yet_for_transaction_purpose()
        {
			...
			{
				$segment_3 = $this -> uri -> segment(3);
				//$data[base_url().'index.php/account_controller/transaction_entry/'.$segment_3.'/sale/'.$field->invoice_id] = $field -> invoice_id;
				$data[base_url().'index.php/account_controller/transaction_entry/'.$segment_3.'/sale/'.$field->invoice_id] = $field -> invoice_id
        ...
        function fatch_all_customerID_not_paid_yet_for_transaction_purpose()
        {
			...
			foreach ($query -> result() as $field)
			{
				$segment_3 = $this -> uri -> segment(3);
				//$data[base_url().'index.php/account_controller/transaction_entry/'.$segment_3.'/sale/'.$field->invoice_id] = $field -> invoice_id;
				$data[base_url().'index.php/account_controller/transaction_entry/'.$segment_3.'/sale_customer/'.$field->customer_id] = $field -> customer_id
								.nbs(5).' ( Name: ' .$field -> customer_name. ' )'.nbs(5).'Contact :  ( '.$field -> customer_contact_no. ' )';
			}
		...
		function fatch_all_investment_id_for_transaction_purpose()
		{
			...
			{
				 $segment_3 = $this -> uri -> segment(3);
				 $data[base_url().'index.php/account_controller/transaction_entry/'.$segment_3.'/investment/'.$field->investment_id] = $field -> investor_name. '  '.' Mob : '.$field -> investor_contact_no;
			}
			return $data;
		}
		...
		function fatch_all_investor_information()
		{
			...
			{
				 $segment_3 = $this -> uri -> segment(3);
				 $data[base_url().'index.php/account_controller/transaction_entry/'.$segment_3.'/Withdrawal/'.$field->investor_id] = $field -> investor_name. ' '.' Mob : '.$field -> investor_contact_no;
