<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed'); 
	class Report_model extends CI_model{
		...
		* ********************************************/
		...
		function get_purchase_return_info_by_multi()
		{
			$distributor_id= $this->input->post('distributor_id');
			$start_date=$this->input->post('start_date');
			$end_date=$this->input->post('end_date');
			...
			$this->db->select('distributor_info.distributor_name,product_info.product_name,purchase_return_details_tbl.purchase_return_id,purchase_return_details_tbl.product_id,purchase_return_details_tbl.return_quantity,purchase_return_details_tbl.unit_buy_price,purchase_return_details_tbl.return_doc,purchase_return_receipt_tbl.status');
			$this->db->from('product_info,purchase_return_details_tbl,purchase_return_receipt_tbl,distributor_info');
			$this->db->where('purchase_return_details_tbl.purchase_return_id = purchase_return_receipt_tbl.purchase_return_id');
			$this->db->where('purchase_return_details_tbl.product_id = product_info.product_id');
			$this->db->where('purchase_return_receipt_tbl.distributor_id = distributor_info.distributor_id');
			
			if($distributor_id!=''){$this->db->where('purchase_return_receipt_tbl.distributor_id',$distributor_id);}
			if($start_date!=''){$this-> db -> where('purchase_return_details_tbl.return_doc >= "'.$start_date.'"');}
			if($end_date!=''){$this -> db -> where('purchase_return_details_tbl.return_doc <= "'.$end_date.'"');}
			else if($start_date!=''){$this -> db -> where('purchase_return_details_tbl.return_doc <= "'.$start_date.'"');}
			
			

			//$this->db->group_by('sale_return_details_tbl.id');
			$this->db->order_by('purchase_return_details_tbl.id','asc'); 
			$this->db->order_by('purchase_return_details_tbl.return_doc','asc'); 
			$query = $this->db->get();
			
			return $query;	
		} 	
		function print_data_purchase_return()
		{
			...
			if($distributor_id!='' && $distributor_id!= 'null'){$this->db->where('purchase_return_receipt_tbl.distributor_id',$distributor_id);}
			if($start_date!='' && $start_date!='null'){$this-> db -> where('purchase_return_details_tbl.return_doc >= "'.$start_date.'"');}
			if($end_date!='' && $end_date!= 'null'){$this -> db -> where('purchase_return_details_tbl.return_doc <= "'.$end_date.'"');}
			else if($start_date!='' && $start_date!= 'null'){$this -> db -> where('purchase_return_details_tbl.return_doc <= "'.$start_date.'"');}
