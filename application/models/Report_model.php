...
class Report_model extends CI_model
{
	...
	public function get_stock_info_by_multi(
		...
		if ($category_id != 0) {
			$this->db->where('product_info.catagory_id = ' . $category_id);
		}
		if ($company_id != 0) {
			$this->db->where('product_info.company_id = ' . $company_id);
		}
		if ($product_amount != 0) {
			$this->db->where('bulk_stock_info.stock_amount <= ' . $product_amount);
		}
		if ($product_size != 'null' && $product_size != '') {
			$this->db->like('product_info.product_size', $product_size);
		}
		if ($product_model != 'null' && $product_model != '') {
			$this->db->like('product_info.product_model', $product_model);
		}
	...
	public function get_purchase_info_by_multi($receipt_id = '', $product_id = '', $distributor_id = '', $start_date = '', $end_date = '', $category1 = '', $company1 = '')
	{
		...
		if ($receipt_id != 0) {
			$this->db->where('purchase_receipt_info.receipt_id', $receipt_id);
		}
		if ($product_id != 0) {
			$this->db->where('product_info.product_id', $product_id);
		}
		if ($category1 != 0) {
			$this->db->where('product_info.catagory_id', $category1);
		}
		if ($company1 != 0) {
			$this->db->where('product_info.company_id', $company1);
		}
		if ($distributor_id != 0) {
	...
	public function get_sale_info_by_multi($invoice_id = '', $customer_id = '', $product_id = '', $seller_id = '', $start_date = '', $end_date = '', $company_id = '', $category_id = '')
	{
		...
		if ($seller_id != 0) {
			$this->db->where('invoice_info.invoice_creator', $seller_id);
		}
		if ($start_date != 0) {
	...
	public function get_quotation_info($data)
	{
		...
		if ($data['quotation_id'] != 0) {
			$this->db->where('quotation_info.quotation_id', $data['quotation_id']);
		}
		if ($data['seller_id'] != 0) {
			$this->db->where('quotation_info.quotation_creator', $data['seller_id']);
		}
		if ($data['customer_id'] != 0) {
			$this->db->where('quotation_info.customer_id', $data['customer_id']);
		}
		if ($data['start_date'] != '') {
			$this->db->where('quotation_info.created_at >= "' . $data['start_date'] . ' 00:00:00"');
		}
		if ($data['end_date'] != '') {
	...
	public function get_other_expense_details($start_date, $end_date)
	{
		...
		if ($start_date != '') {
			$this->db->where('expense_info.expense_doc >= "' . $start_date . '"');
		}
		if ($end_date != '') {
			$this->db->where('expense_info.expense_doc <= "' . $end_date . '"');
		}
		$query1 = $this->db->get();
	...
	public function specific_date_sale_price_calculation($start, $end)
	{
		...
		$query = $this->db->select('SUM(total_price - discount_amount) as total_sale')
			->from('invoice_info')
			->where('invoice_doc >= "' . $start . '"')
			->where('invoice_doc <= "' . $end . '"')
			->get();
	...
	public function get_delivery_charge_info_by_multi()
	{
		...
		if ($start_date != '') {
			$this->db->where('transaction_info.date >= "' . $start_date . '"');
		}
		if ($end_date != '') {
			$this->db->where('transaction_info.date <= "' . $end_date . '"');
		} else if ($start_date != '') {
			$this->db->where('transaction_info.date <= "' . $start_date . '"');
		}
	...
	public function get_purchase_return_info_by_multi($distributor_id, $start_date, $end_date)
	{
		...
		if ($distributor_id != '' && $distributor_id != 'null') {
			$this->db->where('purchase_return_main_product.distri_id', $distributor_id);
		}
		if ($start_date != '' && $start_date != 'null') {
			$this->db->where('purchase_return_main_product.doc >= "' . $start_date . '"');
		}
		if ($end_date != '' && $end_date != 'null') {
			$this->db->where('purchase_return_main_product.doc <= "' . $end_date . '"');
	...
	public function get_expense_info_by_multi()
	{
		...
		if ($expense_type != '') {
			$this->db->where('expense_info.expense_type', $expense_type);
		}
		if ($service_provider_id != '') {
			$this->db->where('expense_info.service_provider_id', $service_provider_id);
		}
		if ($start_date != '') {
			$this->db->where('expense_info.expense_doc >= "' . $start_date . '"');
		}
	...
	public function get_expense_info_by_multi_new()
	{
		...
		$this->db->where('expense_info.expense_type = type_info.type_id');
		if ($type_id != '') {
			$this->db->where('expense_info.expense_type', $type_id);
		}
		if ($start_date != '') {
			$this->db->where('expense_info.expense_doc >= "' . $start_date . '"');
		}
