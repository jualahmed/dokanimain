<?php if(! defined('BASEPATH')) exit('No direct script access allowed'); 
	class Modify_controller extends CI_controller{
		...
		* Ovi
		************************/
		public function bank_modify()
		{
			$data['user_type'] = $this->tank_auth->get_usertype();
			$data['status'] = '';
			$data['user_name'] = $this->tank_auth->get_username();
			$data['records'] = $this -> modify_model -> get_bank_info_modify();
			$this->load->view('Modify/bank_modify',$data);
		}
		function bank_info_edit()
		{
			$id = $this->input->post('id');
			if($id!='')
			{
				$d['records'] = $this -> modify_model -> specific_bank_new($id);
			}
			echo json_encode($d);
		}
		function save_bank_info_edit()
		{
			$hid = $this->input->post('hid');
			
			$this->modify_model->save_bank_info_edit($hid);
			$d = $this -> modify_model -> specific_bank_new($hid);
			echo json_encode($d);
		}
		function delete_bank()
		{
			$bank_id = $this->input->post('id');
			$d = $this -> modify_model -> delete_bank($bank_id);
			echo json_encode($d);
		...
		************************/
		public function transaction_modify()
		{
			...
			$data['status'] = '';
			$data['user_name'] = $this->tank_auth->get_username();
			$this->load->view('Modify/transaction_modify',$data);
		}
