...
class Account_controller extends CI_controller{
	...
	function comission_entry()
		{
			...
			if($this -> access_control_model -> my_access($data['user_type'], 'account_controller', 'bank_entry'))
			{
				...
				$data['user_type'] = $this->tank_auth->get_usertype();
				$this -> load -> view('Account/comission_entry', $data);
			}
			else redirect('account_controller/account/noaccess');
		}
		function create_comission()
		{

			$comission_id = $this -> account_model -> create_comission();
			if($comission_id !='')
			{
				$data['status'] = 'success';
				redirect('account_controller/comission_entry/success');
			}	
			else 
			{
				$data['status'] = 'failed';
				redirect('account_controller/comission_entry/failed');
			}	
		}
	function credit_collection_receipt()
	{
		$data['user_type'] = $this->tank_auth->get_usertype();
		if($this -> access_control_model -> my_access($data['user_type'], 'account_controller', 'purchase_receipt_entry'))
		{
			...
			$data['status'] = '';
			$data['user_name'] = $this->tank_auth->get_username();
			$data['bank_info'] = $this -> account_model -> get_bank();
			$data['distributor_info'] = $this -> product_model -> distributor_info();
			$this -> load -> view('Account/credit_collection_receipt', $data);
		}
		else redirect('account_controller/account/noaccess');
	}
	function purchase_payment_receipt()
	{
		$data['user_type'] = $this->tank_auth->get_usertype();
		if($this -> access_control_model -> my_access($data['user_type'], 'account_controller', 'purchase_receipt_entry'))
		{
			...
			$data['bank_info'] = $this -> account_model -> get_bank();
			$data['distributor_info'] = $this -> product_model -> distributor_info();
			$this -> load -> view('Account/purchase_payment_receipt', $data);
		}
		else redirect('account_controller/account/noaccess');
	}
	function expense_payment_receipt()
	{
		$data['user_type'] = $this->tank_auth->get_usertype();
		if($this -> access_control_model -> my_access($data['user_type'], 'account_controller', 'purchase_receipt_entry'))
		{
			...
			$this -> load -> view('Account/expense_payment_receipt', $data);
		}
		else redirect('account_controller/account/noaccess');
	}
	...
	function get_single_customer_all_sale_payment_total()
	{
	...
	function cheque_status_report()
	{
		...
		$data['bank_info'] = $this -> account_model -> get_bank();		
		$this -> load -> view('Account/cheque_status_report', $data);
	}
	function all_cheque_status_report_find()
	{
		...
		foreach($temp as $field)
		{
			...
			{
				$sale_ledger_name = $this->account_model->get_sale_ledger_name($ledger_type);
				$temp[$i]['sale_ledger_name'] = $sale_ledger_name->result_array();
			}
			else if($ledger_type=='purchase_payment')
			{
				$purchase_ledger_name = $this->account_model->get_purchase_ledger_name($ledger_type);
				$temp[$i]['purchase_ledger_name'] = $purchase_ledger_name->result_array();
			}
			else if($ledger_type=='expense_payment')
			{
				$expense_ledger_name = $this->account_model->get_expense_ledger_name($ledger_type);
	...
	function bank_transfer()
	{
		...
		$data['service_provider_info'] = $this -> account_model -> service_provider_info();		
		$data['bank_info'] = $this -> account_model -> get_bank();		
		$this -> load -> view('Account/bank_transfer', $data);
	}
	function create_transfer()
	{
		...
		if($transfer_id!=''){
			$data['status'] = 'success';
			redirect('account_controller/bank_transfer/success');
		}
		else{
			$data['status'] = 'failed';
			redirect('account_controller/bank_transfer/failed');
		}	
	}
	function owner_transfer()
	{
		...
		$data['bank_info'] = $this -> account_model -> get_bank();				
		$this -> load -> view('Account/owner_transfer', $data);
	}
	function create_owner_transfer()
	{
		$data['user_type'] = $this->tank_auth->get_usertype();
		$data['status'] = '';
		$timezone = "Asia/Dhaka";
		...
		if($transfer_id!=''){
			$data['status'] = 'success';
			redirect('account_controller/owner_transfer/success');
		}
		else{
			$data['status'] = 'failed';
			redirect('account_controller/owner_transfer/failed');
		}	
	}
	...
	function get_all_bank()
	{
		...
		function bank_entry()
		{
			...
			if($this -> access_control_model -> my_access($data['user_type'], 'account_controller', 'bank_entry'))
			{
				...
				$data['user_name'] = $this->tank_auth->get_username();
				$data['user_type'] = $this->tank_auth->get_usertype();
				$this -> load -> view('Account/bank_entry_form_view', $data);
			}
			else redirect('account_controller/account/noaccess');
		}
		function create_bank()
		{
			...
			{
				$data['status'] = 'exist';
				redirect('account_controller/bank_entry/exist');
			}
			$bank_id = $this -> account_model -> create_bank();
			if($bank_id !='')
			{
				$data['status'] = 'success';
				redirect('account_controller/bank_entry/success');
			}	
			else 
			{
				$data['status'] = 'failed';
				redirect('account_controller/bank_entry/failed');
			}	
		}
		
		function create_bank_new()
		{
		...
		function ledgers()
		{
			...
			$this -> load -> view('Account/ledgers', $data);
		}
		function all_ledger_report_find()
		{
			$timezone = "Asia/Dhaka";
			date_default_timezone_set($timezone);
			$bd_date = date('Y-m-d');
			...
			if($purpose_id ==3)
			{
				...
				while($new_date <= $end)
				{
					...
					/* $delete_payment = $this->account_model->all_delete_payment($new_date,$new_date,$distributor_id);
					if($delete_payment)
					{
						$ledger_list[$new_date]['total_delete_payment'] = $delete_payment->result_array();
					}
					else
					{
						$ledger_list[$new_date]['total_delete_payment'] = array();
					}
					*/
			...
			else if($purpose_id ==1)
			{
				...
				/* $sale_return_total_amount = $this->account_model->sale_return_total_amount($start,$customer_id);

				if($sale_return_total_amount)
				{
					$ledger_list2['total_sale_return_amount'] = $sale_return_total_amount->result_array();
				}
				else
				{
					$ledger_list2['total_sale_return_amount'] = array();
				} */
				if($customer_id!='')
				{
				...
				while($new_date <= $end)
				{
					...
					/* $sale_return_details = $this->account_model->all_sale_return($new_date,$new_date,$customer_id);
					if($sale_return_details)
					{
						$ledger_list2[$new_date]['total_sale_return'] = $sale_return_details->result_array();
					}
					else
					{
						$ledger_list2[$new_date]['total_sale_return'] = array();
					} */
		...
		function all_ledger_report_print()
		{
			...
			$data['ledger_list2'] = $ledger_list2;
			$data['ledger_list3'] = $ledger_list3;
			$data['ledger_list4'] = $ledger_list4;
			$data['ledger_list5'] = $ledger_list5;
			//$this -> load -> view('Account/all_ledger_report_preview',$data);
			
			$html=$this->load->view('Download/download_all_ledger_report',$data, true); 
	    ...
	    function expense_entry()
		{
			...
			$data['employee_info'] = $this -> my_variables_model -> employee_info();
			$this -> load -> view('Account/expense_entry_form_view', $data);
		}
		...
		function create_expense()
		{
			...
			$expense_id = $this -> account_model -> create_expense();
			if($expense_id!='')
			{
				$data['status'] = 'success';
				redirect('account_controller/expense_entry/success');
			}
			else
			{
				$data['status'] = 'failed';
				redirect('account_controller/expense_entry/failed');
			}	
		}
		function create_type()
		{
			...
			else
			{
				...
				else if($this -> account_model -> create_type())
				{
					echo 'success';
				}
				else
		...
		function create_investment()
		{
			...
			{
				$data['status'] = 'success';
				redirect('account_controller/investment_entry/success');
			}
			else
			{
				$data['status'] = 'failed';
				redirect('account_controller/investment_entry/failed');
			}	
		...
		function cash_book_report()
		{
			...
			$data['bd_date'] = $bd_date;
			$data['user_name'] = $this->tank_auth->get_username();
			$this -> load -> view('Account/all_cash_book_report_new', $data);
		}
		...
		function today_transaction()
		{
			...
			$data['bd_date'] = $bd_date;
			$data['user_name'] = $this->tank_auth->get_username();
			$this -> load -> view('Account/today_transaction', $data);
		}
		function  all_today_transaction()
		{
			$transaction = array();
			$transaction2 = array();
			$transaction3 = array();
			...
			echo json_encode(array('transaction'=>$transaction,'transaction2'=>$transaction2,'transaction3'=>$transaction3,'transaction4'=>$transaction4,'transaction5'=>$transaction5,'transaction6'=>$transaction6,'transaction7'=>$transaction7,'transaction_sum'=>$transaction_sum));
		}
		function download_todays_transaction()
		{
			date_default_timezone_set("Asia/Dhaka");
			$bd_date = date('Y-m-d',time());
			...
			$html=$this->load->view('Download/download_todays_transaction',$this->data, true); 

			$this->load->library('m_pdf');
			ob_start();
			$this->m_pdf->pdf 	= new mPDF('utf-8', 'A4');
			$this->m_pdf->pdf->SetProtection(array('print'));
			$this->m_pdf->pdf->SetTitle("Today's Transactions");
			$this->m_pdf->pdf->SetAuthor("Dokani");
			$this->m_pdf->pdf->SetDisplayMode('fullpage');
		...
		function pay_reci_report()
		{
			...
			$data['service_provider_info'] = $this -> account_model -> service_provider_info();		
			$data['bank_info'] = $this -> account_model -> get_bank();		
			$this -> load -> view('Account/pay_reci_report', $data);
		}
