	...
	/* 		}
		...
		function all_stock()
		{
			...
			foreach($query -> result() as $field):
				 $temp[base_url().'admin/by_name_result/'.$field->product_id] = $field -> product_name;
			endforeach;
		...
		function all_stock_details()
		{
			...
			if($data['start_date'] == 'x' && $data['end_date'] == 'x'){
				$config['base_url'] = base_url().'index.php/admin/all_stock_details/x/x/';
			}
			else if($data['start_date'] == '' && $data['end_date'] == ''){
				$config['base_url'] = base_url().'index.php/admin/all_stock_details/x/x/';
			}
			else{
				$config['base_url'] = base_url().'index.php/admin/all_stock_details/'.$this->uri->segment(3).'/'.$this->uri->segment(4).'/';
			}
		...
		function by_catagory()
		{
			...
			foreach ($query-> result() as $field)
			{
					...
					$catagory[base_url().'index.php/admin/by_catagory/'.$temp] = $field -> catagory_name;
			}
        ...
        function by_name_result()
		{
			...
			else
			{
				...
				$this -> table -> set_heading('Serial','Stock ID','Purchase Date');
				$config['base_url'] = base_url().'admin/by_name_result/'. $pro_id ;
				$config['per_page'] = 15;
			    $config['num_links'] = 5;
		...
		function advance_search()
		{
			...
			foreach ($query-> result() as $field)
			{
					...
					//$data['http://localhost/inventory_management/index.php/admin/advance_search/'.$field->catagory_name] = $field -> catagory_name;
				    $catagory[base_url().'index.php/admin/advance_search/'.$temp] = $field -> catagory_name;
		    }
			$data['catagory_id'] = $catagory;
     ...
     /*   function edit_product_name()
		...
		function all_distributor()
		{
			...
			$config['base_url'] = base_url().'index.php/admin/all_distributor/';
		...
		function all_company()
		{
			...
			$config['base_url'] = base_url().'index.php/admin/all_company/';
		...
		function all_catagory()
		{
			...
			$config['base_url'] = base_url().'index.php/admin/all_catagory/';
		...
		function all_registerd_customer()
		{
			...
			$config['base_url'] = base_url().'index.php/admin/all_registerd_customer/';
		...
		function all_customer()
		{
			...
			$config['base_url'] = base_url().'index.php/admin/all_customer/';
			...
			foreach($query -> result() as $field):
				 $temp[base_url().'admin/all_customer/0/'.$field->customer_id] = $field -> customer_name;
			endforeach;
			...
			foreach($query -> result() as $fielded):
				 $tempo[base_url().'admin/all_customer/0/0/'.$fielded->customer_id] = $fielded -> customer_define_id;
			endforeach;
		...
		*--------------------------------------------*/
		function searchBarcode()
		{
			...
			$temp[''] = 'Select A Product';
			foreach($query -> result() as $field):
				 $temp[base_url().'admin/searchBarcode/'.$field->product_id] = $field -> product_name;
			endforeach;
			$data['product_info'] = $temp;
