...
class Product extends MY_Controller
{
	...
	public function status($product_id)
	{
		...
		$data = $this->db->where('product_id', $product_id)
			->set('product_status', $product_status == 1 ? 0 : 1)
			->update('product_info');
		echo json_encode($data);
	}
	...
	public function searchBarcode()
	{
		...
		if (isset($receipt_id) && $receipt_id) {
			...
			$receiptProducts = $this->db->select('purchase_info.*, product_info.product_name')
				->from('purchase_info')
				->join('product_info', 'product_info.product_id=purchase_info.product_id')
				->where('purchase_receipt_id', $receipt_id)
				->get();
			$productIds = [];
			if ($receiptProducts->num_rows() > 0) {
				$products = $receiptProducts->result();
				$data['receiptProducts'] = $products;
				$productIds = array_map(function ($product) {
					return $product->product_id;
				}, $products);
				$data['product_info']  = $this->product_model->productsss_info(true, $productIds, $this->tank_auth->get_shop_id())->result();
			}
		} else {
		...
		else $product_ID = $product_id; // from url
