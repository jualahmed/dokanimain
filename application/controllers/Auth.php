		...
		if ($this->tank_auth->is_logged_in()) 
		{
			...
			if ($this->tank_auth->is_max_login_attempts_exceeded($login)) {
				if ($data['use_recaptcha'])
					...
					$this->form_validation->set_rules('captcha', 'Confirmation Code', 'trim|xss_clean|callback__check_captcha');
			}
			$data['errors'] = array();
	...
	function register()
	{
		...
		if($data['user_type'] == 'superadmin' || $data['user_type'] == 'admin' || $data['user_type'] == 'manager' )
		{
			...
			if($data['user_type'] == 'seller')
					redirect('registration_controller/registration');
