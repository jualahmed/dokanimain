...
class Tank_auth
{
	 ...
	 * Gate Pass Running *****************
	****************************************/
	function set_current_gate_pass_for( $salseman_id)
	{
		$this->ci->session->set_userdata(array(
			'gate_pass_for'	=> $salseman_id
		));
	}
	/*******************************
	 * Get Current Gate Pass for ***
	********************************/
	function get_current_gate_pass_for()
	{
		return $this->ci->session->userdata('gate_pass_for');
	}
	
	/****************************************
	 * Gate Pass Finalizing *****************
	*****************************************/
	function set_gate_pass_finalizing($gate_pass_finalizing_id)
	{
		$this->ci->session->set_userdata(
			array(
				'gate_pass_finalizing_id'	=> $gate_pass_finalizing_id,
			));
	}
	/*******************************
	 * Get CGate Pass Finalizing ***
	********************************/
	function get_gate_pass_finalizing()
	{
		return $this->ci->session->userdata('gate_pass_finalizing_id');
	}
	/****************************************
	 * Set Temporary Current Sale ***************
	*****************************************/
	function set_current_temp_sale( $temp_sale_id )
	{
		$this->ci->session->set_userdata(array('currrent_temp_sale_id' => $temp_sale_id));
	}
	/* Start: (Arun)*/
	...
	{
		$this->ci->session->set_userdata(array('tmp_sale_return_id' => $id));	
	}
	
	function get_current_sale_return_id()
	{
		return $this->ci->session->userdata('tmp_sale_return_id');
	}
	function unset_current_sale_return_id()
	{
		$this->ci->session->unset_userdata('tmp_sale_return_id');
	}
	function set_id_for_purchase_return($id)
	{
		$this->ci->session->set_userdata(array('tmp_purchase_return_id' => $id));	
	}
	function get_current_purchase_return_id()
	{
		return $this->ci->session->userdata('tmp_purchase_return_id');
	}
	function unset_current_purchase_return_id()
	{
		$this->ci->session->unset_userdata('tmp_purchase_return_id');
	}
	/*End: (Arun)*/
	...
	********************************/
	...
	{
		return $this->ci->session->userdata('shop_id');
	}
	function get_shop_name()
	{
		return $this->ci->session->userdata('shop_name');
	}
	...
	function get_shopcontact()
	{
		return $this->ci->session->userdata('shop_contact');
	}
	 ...
	 * @return	string
	 */
	function get_shopaddress()
	{
		return $this->ci->session->userdata('shop_address');
	}
	/**
	 ...
	 * @return	array
	 */
	function create_user($username, $full_name, $email, $password, $user_type, $user_address, $assigned_shop, $email_activation)
	{
	...
	function change_password($old_pass, $new_pass)
	{
	 ...
	 /* to update password without using old password */
    function change_password_special($new_pass,$id)
	{
		$name = $id;
		//echo $name; echo $new_pass;
		...
		if (!is_null($user = $this->ci->users->get_user_by_id($id, TRUE))) 
		{
			...
			$hasher = new PasswordHash(
					...
					$this->ci->config->item('phpass_hash_portable', 'tank_auth'));
			 $hashed_password = $hasher->HashPassword($new_pass);
			if ($this->ci->users->change_password_special($id, $hashed_password))
			{			// success
				...
				// Replace old password with new one
				//$this->ci->users->change_password_special($name, $hashed_password);
	 ...
	 * Arafat Mamun
	 *********************/
	function alter_shop($shop_id)
	{
		$query = $this->ci->users->shop_information(TRUE, $shop_id);
		$this->ci->session->set_userdata(array(
								'shop_name' => $query -> shop_name,
								'shop_type'=> $query -> shop_type,
								'shop_address' => $query -> shop_address,
								'shop_id' => $query -> shop_id,
								'shop_contact' => $query -> shop_contact
							));
	}
}
