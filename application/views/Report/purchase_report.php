								...
								<div class="col-sm-2">
									<select id="receipt_id" name="receipt_id" class="form-control" v-model="receipt">
										...
										<?php foreach ($purchase_receipt as $key => $value) : ?>
											<option value="<?php echo $value->receipt_id ?>"><?php echo $value->receipt_id ?> (<?php echo $value->distributor_name ?>)</option>
										<?php endforeach ?>
									</select>
								</div>
		...
		<div class="row">
			<div class="col-md-12">
				<div id="table-scroll" class="table-scroll table-secondary table-responsive" v-if="alldata.length>0">
					<table id="main-table" class="main-table table table-secondary" style="width: 100%;min-width: 100%;">
						<thead class="table-hf" style="line-height: 0px;">
							<tr>
								<th>No</th>
								<th>Date</th>
								<th title="Receipt ID">Receipt</th>
								<th>Company</th>
								<th>Category</th>
								<th>Product</th>
						...
						<tbody>
							<tr v-for="(a,index) in alldata">
								...
								<th style="white-space: normal!important;">{{ a.product_name }}</th>
								<td class="text-center">{{ a.purchase_quantity }}</td>
								<td class="text-right">{{ parseFloat(a.unit_buy_price).toFixed(2) }}</td>
							</tr>
							<tr>
								<td colspan="6"><b>Total</b></td>
								<td class="text-right"><b>{{ stockqty }}</b> </td>
								<td class="text-right"><b>{{ parseFloat(samount).toFixed(2) }}</b></td>
							</tr>
						</tbody>
					</table>
				</div>
