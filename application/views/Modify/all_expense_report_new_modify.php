		...
		<div class="row">
			<div class="col-md-10 col-md-offset-1">
				<div class="box">
					...
					<div class="box-body">
						<form action ="<?php echo base_url();?>modify_controller/all_expense_report_find_new" class="form-horizontal" method="post" enctype="multipart/form-data" id="form_4" autocomplete="off">
		...
		$.ajax({
			...
			success: function(result) {	
				...
				for(i=0; i<result.length; i++){	  
					output+='<table class="new_data_2"><tr><td>'+k+'</td><td>'+result[i].expense_id+'</td><td title="'+result[i].expense_type+'">'+result[i].type_type+'</td><td title="'+result[i].expense_amount+'" >'+result[i].expense_amount+'</td><td title="'+result[i].expense_doc+'">'+result[i].expense_doc+'</td><td style="text-align:center;"><a class="btnDelete" style="cursor:pointer;"><i class="fa fa-fw fa-close"></i></a></td></tr></table>';
					k++;
				}
				if(output != '')
				{
...
function deleter(par) 
{
	...
	var urlx='<?php echo base_url();?>modify_controller/delete_expense_info';
	var id=td_Id.html();
	 $.ajax({
		url: urlx,
		type: 'POST',
