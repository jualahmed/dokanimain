<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');?>
    ...
    <div class="mid_box_top">
		...
		<?php
			if($action == 'modify') 
				echo form_open('modify_controller/sale_modify');
			else echo form_open('report_controller/sale_return_report');
		...
		<table >
			...
			<tr>
		        ...
		        <td>
					 <?php
							if($action == 'modify')
								echo anchor('modify_controller/generate_invoice_details/'.$field -> invoice_id, img('images/megnify_2.png') ,'class = "table_img" target="_blank"');
							else echo anchor('expense_invoice_controller/generate_invoice/'.'old_invoice/'.$field -> invoice_id, img('images/megnify_2.png') ,'class = "table_img" target="_blank"');
					?>
				</td>
		    </tr>
			<?php
