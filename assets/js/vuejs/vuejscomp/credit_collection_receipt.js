  ...
  $("#customer_id").on("change", function () {
    ...
    if (customer_id != "") {
      $(".collection_payment").fadeIn("slow", function () {
        var urlx =
          base_url + "account/get_single_customer_all_sale_payment_total";
        var outputs = "";
        $.ajax({
          url: urlx,
          type: "POST",
          ...
          success: function (result) {
            for (
              n = 0;
              n < result.ledger_list["receipt_sale_total_amount"].length;
              n++
            ) {
              var sale_amount_all = parseFloat(
                result.ledger_list["receipt_sale_total_amount"][n][
                  "total_sale_amount"
                ]
              ).toFixed(2);
            ...
            for (
              m = 0;
              m < result.ledger_list["receipt_collection_total_amount"].length;
              m++
            ) {
              var collection_amount_all = parseFloat(
                result.ledger_list["receipt_collection_total_amount"][m][
            ...
            for (
              mm = 0;
              mm < result.ledger_list["receipt_delivery_total_amount"].length;
              mm++
            ) {
              var delivery_amount_all = parseFloat(
                result.ledger_list["receipt_delivery_total_amount"][mm][
            ...
            for (
              m = 0;
              m < result.ledger_list["receipt_sale_return_total_amount"].length;
              m++
            ) {
              var sale_return_amount_all = parseFloat(
                result.ledger_list["receipt_sale_return_total_amount"][m][
            ...
            for (
              im = 0;
              im <
              result.ledger_list["receipt_cash_return_total_amount"].length;
              im++
            ) {
              var cash_return_amount_all = parseFloat(
                result.ledger_list["receipt_cash_return_total_amount"][im][
                  "total_cash_return_amount"
                ]
              ).toFixed(2);
            }
            for (
              j = 0;
              j < result.ledger_list["receipt_balance_total_amount"].length;
              j++
            ) {
              var balance_amount_all = parseFloat(
                result.ledger_list["receipt_balance_total_amount"][j][
            ...
            var due_amount =
              ...
              parseFloat(cash_return_amount_all) +
      ...
      if (payment_mode != "" && receipt_type != "" && payment_input != "") {
        $.ajax({
          ...
          success: function (result) {
            ...
            swal("Oops...!", "Data Insert Successfully!", "success");
            location.reload();
            if (result != "cheque") {
              window.open(
                base_url +
                  "invoice/collection_payment_invoice/" +
                  result +
                  "/" +
                  receipt_type,
                "_blank"
