...
jQuery(document).ready(function ($) {
  ...
  $("#search_by_product_name").on("keyup", function (ev) {
    ...
    if (is_quotation_active && $(this).val().length > 1) {
      ...
      if (ev.keyCode == 13) {
        $.ajax({
          url: base_url + "sale/searchProductByBarcodeForQuotation",
          dataType: "json",
          type: "POST",
          cache: false,
        ...
        $("#search_by_product_name").autocomplete({
          source: function (request, response) {
            $.ajax({
              url: base_url + "sale/searchProductForQuotation",
              dataType: "json",
              type: "POST",
              cache: false,
  ...
  function showSelectedProduct(item) {
    ...
    $("#product_price").val(new_sale_price);
  ...
  $("#product_price").on("keydown", function (e) {
    if (e.keyCode === 13) {
      addToQuotation();
    }
  });
    ...
    if (product_specification == 2) {
      ...
      $("#add_product_serial_form").on("submit", function (service) {
        ...
        if (isNaN(parseFloat(product_price)) || parseFloat(product_price) < 0) {
          setTimeout(() => {
            swal("Oops...!", "Please inter valid sale price", "error");
            $("#product_price").focus();
          }, 500);
        } else {
      ...
      if (isNaN(parseFloat(product_price)) || parseFloat(product_price) < 0) {
        ...
        sale_price = $("#product_price").val();
        sale_price = parseFloat(sale_price); // unit_sale_price
        var temp_data = $("#temp_pro_data").val();
        num_of_tr += 1;
        var tmp_amount = rstock * pro_price;
